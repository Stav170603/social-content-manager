...
function ContentMediaCarousel({ media = [], fallbackUrl, fallbackType, alt = 'תוכן' }) {
  ...
  return <section className="content-media-carousel" aria-label={`קרוסלת מדיה, ${items.length} פריטים`}>
    <MediaPreview key={`${item.mediaId || item.media_id || safeIndex}-${item.mediaUrl || item.media_url || ''}`} path={item.mediaUrl || item.media_url} type={item.mediaType || item.media_type} poster={item.thumbnailUrl || item.thumbnail_url} alt={`${alt}, פריט ${safeIndex + 1}`} />
